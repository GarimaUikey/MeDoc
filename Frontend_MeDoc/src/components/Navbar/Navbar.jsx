  ...
  return (
    ...
    <div className='navbar'>
        ...
        <ul className='hidden md:flex items-start gap-5 font-medium text-[#00173d]'>
            ...
            <NavLink to='/OrderNow'>
                <li className='py-1'>ORDER NOW</li>
                <hr className='border-none outline-none h-0.5 w-3/5 m-auto'/>
            </NavLink>
            <NavLink to='/About'>
            ...
            <div className='flex items-center gap-4'>
                ...
                <div className={` ${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
                    <div className='flex items-center justify-between px-5 py-6'>
                        <img style={{width: '150px'}} src={assets.logo} alt="" />
                        <img className='w-7' onClick={()=>setshowMenu(false)} src={assets.cross_icon} alt="" />
                    </div>
                    <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
                       <NavLink  onClick={()=>setshowMenu(false)} to='/'><p className='px-4 py-2 rounded inline-block '> HOME</p></NavLink>
                       <NavLink  onClick={()=>setshowMenu(false)} to='/Doctors'><p className='px-4 py-2 rounded inline-block '>ALL DOCTORS</p></NavLink>
                       <NavLink  onClick={()=>setshowMenu(false)} to='/OrderNow'><p  className='px-4 py-2 rounded inline-block '> ORDER NOW</p></NavLink>
